...
export default function AboutPage() {
  return (
    <div className="overflow-hidden pt-24">
      ...
      <section className="relative py-20 md:py-28">
        ...
        <div className="container relative z-10">
          <motion.div {...fadeInUp} className="max-w-2xl">
            <Badge variant="primary" className="mb-4" style={{ background: "rgba(15,157,148,0.2)", color: "#5eead4", borderColor: "rgba(15,157,148,0.3)" }}>About Us</Badge>
            <h1 className="font-display text-4xl md:text-5xl font-bold text-white mb-6 text-balance">Learn More About Us</h1>
              ...
              <div className="absolute -bottom-6 -right-6 rounded-2xl p-5 shadow-large" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm" style={{ color: "var(--text-muted)" }}>Excellence in Real Estate</p>
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      <section className="py-20 md:py-28" style={{ background: "var(--bg-alt)" }}>
        <div className="container">
          <motion.div {...staggerContainer} className="grid md:grid-cols-2 gap-8">
            <motion.div {...fadeInUp} className="rounded-2xl p-8 shadow-soft" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
              <div className="flex items-center justify-center w-14 h-14 rounded-xl mb-5" style={{ background: "var(--primary-light)", color: "var(--primary)" }}>
                <Target className="h-6 w-6" />
              </div>
              <h3 className="font-display text-2xl font-bold mb-4" style={{ color: "var(--text)" }}>Our Mission</h3>
            ...
            <motion.div {...fadeInUp} className="rounded-2xl p-8 shadow-soft" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
              ...
              <h3 className="font-display text-2xl font-bold mb-4" style={{ color: "var(--text)" }}>Our Vision</h3>
              <p className="leading-relaxed" style={{ color: "var(--text-secondary)" }}>
      ...
      <section className="py-20 md:py-28" style={{ background: "var(--bg)" }}>
        <div className="container">
          <motion.div {...fadeInUp} className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h2 className="font-display text-3xl md:text-4xl font-bold mb-4 text-balance" style={{ color: "var(--text)" }}>Efficient Property Management</h2>
          ...
          <motion.div {...staggerContainer} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {WHY_CHOOSE_US.map((item) => (
              <motion.div key={item.id} {...fadeInUp}>
                <div className="h-full p-6 rounded-2xl transition-all duration-300" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                  <div className="flex items-center justify-center w-12 h-12 rounded-xl mb-4" style={{ background: "var(--primary-light)", color: "var(--primary)" }}>
      ...
      {/* Stats */}
      <section className="py-16" style={{ background: "linear-gradient(to right, var(--primary), var(--primary-hover))" }}>
        <div className="container">
          <motion.div {...staggerContainer} className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {STATISTICS.map((stat) => (
              <motion.div key={stat.id} {...fadeInUp} className="text-center">
                ...
                <p className="text-sm mt-2" style={{ color: "rgba(255,255,255,0.8)" }}>{stat.label}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <section className="py-20 md:py-28" style={{ background: "var(--bg-alt)" }}>
        <div className="container">
          <motion.div {...fadeInUp} className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h2 className="font-display text-3xl md:text-4xl font-bold mb-4 text-balance" style={{ color: "var(--text)" }}>Meet Our Leadership Team</h2>
                ...
                <motion.div whileHover={{ y: -6 }} className="h-full rounded-2xl overflow-hidden transition-all duration-300 group" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                  <div className="relative aspect-[4/5] overflow-hidden">
                    ...
                    <div className="absolute bottom-4 left-4 right-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-4 group-hover:translate-y-0">
                      ...
                      {member.twitter && <a href={member.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-9 h-9 rounded-full text-white hover:bg-white/40 transition-all" style={{ background: "rgba(255,255,255,0.2)", backdropFilter: "blur(4px)" }} aria-label="Twitter"><SocialIcon type="twitter" className="h-4 w-4" /></a>}
                      {member.instagram && <a href={member.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-9 h-9 rounded-full text-white hover:bg-white/40 transition-all" style={{ background: "rgba(255,255,255,0.2)", backdropFilter: "blur(4px)" }} aria-label="Instagram"><SocialIcon type="instagram" className="h-4 w-4" /></a>}
                    </div>
                  </div>
                  <div className="p-5">
                    <h3 className="font-semibold" style={{ color: "var(--text)" }}>{member.name}</h3>
                    <p className="text-sm font-medium mb-2" style={{ color: "var(--primary)" }}>{member.role}</p>
                    <p className="text-xs line-clamp-3 leading-relaxed" style={{ color: "var(--text-muted)" }}>{member.bio}</p>
                  </div>
                </motion.div>
              </motion.div>
            ))}
      ...
      <section className="py-20 md:py-28" style={{ background: "var(--bg)" }}>
        <div className="container">
          <motion.div {...fadeInUp} className="rounded-3xl p-8 md:p-12 text-center relative overflow-hidden" style={{ background: "linear-gradient(to right, var(--primary), var(--primary-hover))" }}>
            ...
            <div className="relative z-10">
              ...
              <p className="text-lg mb-8 max-w-xl mx-auto" style={{ color: "rgba(255,255,255,0.8)" }}>Let our expert team help you find the perfect property or maximize your real estate investment.</p>
